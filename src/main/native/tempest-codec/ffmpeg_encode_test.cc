...
bool video_encode_receive_packets(
    std::ostream& out,
...
bool make_image(int tt, AVFrame* frame) {
    frame->pts = tt;
...
void video_encode_example1() {
    std::ofstream fout("test.h264", std::ios::out | std::ios::binary);
    video_encode_example(fout, make_image, AV_CODEC_ID_H264);
    fout.close();
}
